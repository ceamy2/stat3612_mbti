...
cancerfactor<-cancer[,3:12]
## there is no logic why we exclude 13~32.need explanation for that  
...
```{r}
radius_mean <- ggplot(cancer,aes(x=factor(diagnosis),y=radius_mean)) + geom_boxplot() + geom_smooth(method="lm",se=TRUE, fullrange=FALSE, level=0.95)
plot(radius_mean)
...
set.seed(12L) 
original_data$diagnosis=ifelse(original_data$diagnosis==0,"No","Yes")
trainingIndex <-createDataPartition(original_data$diagnosis, p= 0.7, list=FALSE)
trainingData<-original_data[trainingIndex,]
testData<-original_data[-trainingIndex,]
