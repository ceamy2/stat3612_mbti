...
fviz_pca_ind(cancer.pca, geom.ind = "point", pointshape=21, pointsize=2, fill.ind=cancer$diagnosis, addEllipses = TRUE, label= "var", col.ind = "black", col.var="black", paleete="jco", legend.title ="Diagnosis" ) + ggtitle("two dimensional PCA plot based on 30 features")+ theme(plot.title = element_text(hjust=0.5))
...
smp_size<-floor(0.75*nrow(cancer.pcst))


pca_ldf<- lda(diagnosis~ cancer.pcst[1]+ cancer.pcst[2],data=trainingData)
...
dataset=data.frame(species= cancer[,"diagnosis"],pca=cancer.pca$x,lda=plda$x)

lda_df
p1 <- ggplot(dataset) + geom_point(aes(lda_df.LD1, lda_df.LD2, colour = species, shape = species), size = 2.5) + 
  labs(x = paste("LD1 (", percent(prop.lda[1]), ")", sep=""),
       y = paste("LD2 (", percent(prop.lda[2]), ")", sep=""))

p1

topterm<-preproc_lda_df %>%
        group_by(cancerfactor)%>%
        top_n(15,beta) %>%
        ungroup() %>%
        arrange(cancerfactor,-beta)
...
#give the plot and the AUC value in R console 
prediction_pknn
knn.roc=performance(prediction_pknn,"tpr","fpr")
knn.auc=performance(prediction_pknn,"auc")
